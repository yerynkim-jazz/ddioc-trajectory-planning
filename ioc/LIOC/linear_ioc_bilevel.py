...
def _maybe_add_repo_root_to_syspath() -> None:
    ...
    script directory (ioc/LIOC), which may prevent absolute imports like
    `import ioc.DDIOC.*` from working. We detect the repo root by searching
    upwards for `ioc/DDIOC/tune_qp_planner.py` and prepend that directory to
    sys.path.
    """
    ...
    for parent in (this_file.parent,) + tuple(this_file.parents):
        if (parent / "ioc" / "DDIOC" / "tune_qp_planner.py").is_file():
